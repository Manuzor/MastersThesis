...
\label{cha:RenderPipeline}
  ...
  \label{sec:BuildCommandBuffers}
    ...
    % \subsection{Rendering a Scene Object}
      ...
      \proofread
      \todo[fancyline,color=green!60]{Describe why performance may be affected negatively? Or remove it.}
  ...
  \label{sec:MultithreadedRendering}
    ...
    \subsection{Multi-threaded Command Buffer Recording}
      ...
      \subsubsection{Command Buffer Management Considerations}
        ...
        In practice, this effectively means command buffers submitted in batches may only be modified or reused once the entire submission has finished execution \todo[fancyline,color=green!60]{Why not use event at end of command buffer?}because it is hard to predict when individual commands are processed if more than one command buffer is submitted.
