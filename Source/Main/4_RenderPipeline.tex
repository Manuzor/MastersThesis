...
\label{cha:RenderPipeline}
  ...
  \label{sec:BuildCommandBuffers}
    ...
    An example for such kinds of command buffers is for rendering static scene geometry.
    ...
    If values are stored in the push constant block that change every frame, such as the \gls{mvpmatrix} matrix of a scene object, command buffers need to be recorded every frame.
