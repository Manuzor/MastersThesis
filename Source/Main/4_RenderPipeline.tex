...
\label{cha:RenderPipeline}
  ...
  \label{sec:RenderingSetup}
    ...
    \subsection{Creating a Graphics Pipeline}
      \todo[inline]{Introduction to what a (graphics) pipeline is and why it is useful.}
      \todo[inline]{Optional dynamic state (line width, viewport, etc...)}
      \todo[inline]{Create framebuffer for each swapchain image, a render pass, and graphics pipeline. }
      \todo[inline]{Mention PSO creation is costly. Viewport is dynamic state because of window resizing.}
      \todo[inline]{Sub-sub-sections here are derived from what a VkGraphicsPipelineCreateInfo expects. Mention this?}
      \tbd
      ...
      \subsubsection{Viewport State}
        ...
        \todo{Mention GPU feature that has to be enabled if more than 1 viewport is required?}In this chapter, only one of each is used.
      ...
      \subsubsection{Render Pass}
        ...
        The following settings are used to control the image layout at the beginning and the end of a render pass, respectively.
