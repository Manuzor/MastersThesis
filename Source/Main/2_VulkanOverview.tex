...
\label{cha:VulkanOverview}
  ...
  \label{sec:CommandBuffers}
    ...
    \todo{Briefly explain initial, record, and execute states.}In the \textit{initial} state, the internally managed state of the command buffer is undefined. From the \textit{initial} state, a command buffer can be brought into the \textit{recording} state. In this state the command buffer accepts commands to be recorded to the internally managed state. Once recording of all commands is done, the command buffer can be brought to the \textit{executable} state. In this state, the command buffer may be submitted to a queue which will execute the recorded commands on the GPU. Without implicit dependencies or explicit synchronization, the order in which commands are executed on the GPU is not defined. This means that submitted commands may run concurrently or in a different order than they were recorded in.
