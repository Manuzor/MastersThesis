...
\label{cha:VulkanOverview}
  ...
  \label{sec:WorkflowAndPatterns}
    ...
    The Vulkan instance can be used to query available physical \glspl{device} that provide Vulkan support.
    ...
    The number of \glspl{device} created from a physical device is not limited.

    Each logical device is associated with a number of device queues.
    A device queue, or just queue, is used to submit work to the \gls{gpu}.
    They are logical instances of what is called a queue family.
    ...
    When creating a logical device, all queues associated with it are created as well.
