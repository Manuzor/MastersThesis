...
\label{cha:EnvSetup}
  ...
  \label{sec:VulkanHeader}
    ...
    The \lstinline{vkspec.xml} essentially contains annotated C code with XML markup that is \todo{"extract" is better}easier to parse than pure C code.
  ...
  \label{sec:VulkanLoader}
    ...
    However, because of the \gls{vkloader} being open-source software, an \todo{this is meant to be the "developer"}application could extract the relevant loader logic, customizing it to their specific needs.
    ...
    \todo[inline]{Performance with layers.
    What to do in release builds: get device addresses, deactivate layers, etc...}
    ...
    \todo[inline]{Device extensions are for vendor-specific extensions, such as NV\_glsl that support loading glsl shader code.
    For more on shaders see sec:EnvShaders.}

    \todo[inline]{Layers are discovered by the loader via the registry on Windows and in special directories on Unix based platforms.
  ...
  \label{sec:EnvShaders}
    ...
    \todo{rewrite this sentence? sounds funny...} A common way to write shaders for Vulkan applications is by first writing them in \gls{glsl} and then converting these shaders to \gls{spirv} using a special tool.
  ...
  \label{sec:DebuggingAndProfiling}
    ...
    It also provides a layer which combines all debugging layers in one layer in an optimal order called \todo{Mention the \lstinline{VK_LAYER_LUNARG_} prefix?}\lstinline{standard_validation}.
    ...
    \todo{Find out where this extension is coming from?}For this purpose the \lstinline{VK_EXT_debug_report} extension is used.
    ...
    \todo{Is this comprehensive enough?}This gives applications the opportunity to implement their own \gls{gpu} profiling system by taking time samples at application-defined points in a commandbuffer.
